...
class BebidaBase(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=100, description="Nombre de la bebida"
    )
    description: Optional[str] = Field(
        None, max_length=200, description="Descripción de la bebida"
    )
    price: float = Field(
        ..., gt=0, description="Precio de la bebida (debe ser mayor a 0)"
    )
    available: bool = Field(
        default=True, description="Disponibilidad de la bebida"
    )
    category: Optional[str] = Field(
        None, max_length=50, description="Categoría de la bebida (opcional)"
    )
    stock: Optional[int] = Field(
        0, ge=0, description="Cantidad en stock (opcional)"
    )
    ...
    def validate_available(cls, value):
        ...
        if isinstance(value, bool):
            return value
        raise ValueError(
            "El campo 'available' debe ser booleano (True/False) "
            "y no admite strings."
        )
